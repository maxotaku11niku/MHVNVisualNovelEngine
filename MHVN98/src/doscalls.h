...
#define int21_readfile(handle, len, bufseg, buffer, readbytes, errorflag) asm inline ("movw %w2, %%ds\n\tmovb $63, %%ah\n\tint $33\n\tmovw %%cs, %w2\n\tmovw %w2, %%ds" : "=a" (readbytes), "=@ccc" (errorflag), "+r" (bufseg), "=m" (*((unsigned char*)buffer)) : "b" (handle), "c" (len), "d" (buffer))
...
#define int21_writefile(handle, len, bufseg, buffer, writebytes, errorflag) asm inline ("movw %w2, %%ds\n\tmovb $64, %%ah\n\tint $33\n\tmovw %%cs, %w2\n\tmovw %w2, %%ds" : "=a" (writebytes), "=@ccc" (errorflag), "+r" (bufseg) : "b" (handle), "c" (len), "d" (buffer), "m" (*((unsigned char*)buffer)))
